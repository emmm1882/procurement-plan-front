...
接口文档：
项目启动成功后访问：http://localhost:8080/api/swagger-ui/index.html
...
![创建采购计划](./images/创建采购计划.png)

#### 3.1.2 查询采购计划列表

- 请求路径：`/api/procurement-plans/page`
- 请求方式：GET
- 请求参数：

![查询采购计划列表](./images/查询采购计划列表.png)
...
```json
{
    ...
    "data": {
        ...
        "list": [
            {
                "id": "4aa0583b101a674bfa62edd69ed86e46",
                "planName": "2024年度采购计划",
                "year": "2024年/年度",
                "company": "测试人员单位",
                "dept": "测试一部",
                ...
                "status": "SAVED",
                "createUserId": null,
                "createDate": "2025-06-16T14:49:05.000+00:00",
                "updateUserId": null,
                "updateDate": "2025-06-16T14:49:05.000+00:00",
                "isDeleted": 0
            },
            {
                "id": "8e1a89b242e569b07be8e1bbc71da582",
                "planName": "2024年度采购计划",
                "year": "2024年/年度",
                "company": "测试人员单位",
                "dept": "测试一部",
                ...
                "status": "SAVED",
                "createUserId": null,
                "createDate": "2025-06-16T14:52:09.000+00:00",
                "updateUserId": null,
                "updateDate": "2025-06-16T14:52:09.000+00:00",
                "isDeleted": 0
            }
        ]
    }
...
#### 3.1.4 删除采购计划
- 请求路径：`api/procurement-plans/{id}`

- 请求方式：DELETE

- 响应结果：

  ![删除采购计划](./images/删除采购计划.png)
```json
{
    "msg": "success",
    "code": 0
...
#### 3.2.1 修改计划状态
- 请求路径：`api/procurement-plans/{id}/status`

- 请求方式：PUT

- 请求参数：

  ![修改计划状态](./images/修改计划状态.png)
...
1. **创建采购计划**
   - 用户通过前端页面填写采购计划信息，包括计划名称、所属年度、所属公司、编制部门、编制人、编制时间等。
   - 用户可添加采购明细，包括采购名称、采购类别、采购方式、拟采购估价、计划采购时间、资金来源、备注等。
   - 用户点击"保存"按钮，系统调用创建采购计划接口（`/api/procurement-plans`，POST），将数据保存到数据库。

2. **查询采购计划列表**
   - 用户通过前端页面输入查询条件（如计划名称、状态等），点击"查询"按钮。
   - 系统调用查询采购计划列表接口（`/api/procurement-plans/page`，GET），返回符合条件的采购计划列表。

3. **修改采购计划**
   - 用户在采购计划列表中选择需要修改的计划，点击"修改"按钮。
   - 系统加载该计划的详细信息，用户修改后点击"保存"按钮。
   - 系统调用修改采购计划接口（`/api/procurement-plans`，PUT），更新数据库中的计划信息。

4. **删除采购计划**
   - 用户在采购计划列表中选择需要删除的计划，点击"删除"按钮。
   - 系统调用删除采购计划接口（`/api/procurement-plans/{id}`，DELETE），从数据库中删除该计划。

5. **Excel导入与导出**
   - 用户可通过Excel导入功能批量添加采购计划，系统调用导入接口（`/api/procurement-plans/import`，POST）。
   - 用户可通过Excel导出功能导出采购计划列表，系统调用导出接口（`/api/procurement-plans/export`，GET）。

6. **文件上传**
   - 用户可上传附件，系统调用文件上传接口（`/api/procurement-plans/upload`，POST），将文件保存到服务器。

7. **更新计划状态**
   - 用户可修改采购计划的状态，系统调用更新状态接口（`/api/procurement-plans/{id}/status`，PUT）。
   - 状态流转规则：
     - "已保存" 只能变为 "审批中" 或 "已删除"
     - "审批中" 只能变为 "已生效" 或 "审批退回"
     - "审批退回" 只能变为 "已保存" 或 "已删除"
...
![核心业务流程图](./images/核心业务流程图.png)

**流程说明：**

1. **创建采购计划**：用户填写计划信息并保存。
2. **查询采购计划**：用户查询计划列表。
3. **修改采购计划**：用户修改计划信息。
4. **删除采购计划**：用户删除计划。
5. **Excel导入导出**：用户批量导入或导出计划。
6. **文件上传**：用户上传附件。
7. **更新计划状态**：用户修改计划状态。
8. **审批流程**：计划进入审批流程。
9. **归档**：计划审批通过后归档。
