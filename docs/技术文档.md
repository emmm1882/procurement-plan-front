...
5. **Excel导入**
   - **请求路径**：`/api/procurement-plans/import`
   - **请求方式**：POST
   - **请求参数**：file（Excel文件，form-data上传）

6. **Excel导出**
   - **请求路径**：`/api/procurement-plans/export`
   - **请求方式**：GET
   - **请求参数**（Query）：
     - planName：计划名称（可选）
     - status：状态（可选）
...
7. **文件上传**
   - **请求路径**：`/api/procurement-plans/upload`
   - **请求方式**：POST
   - **请求参数**：file（文件，form-data上传）
