...
<script>
import { getPlanDetail, getPlanDetailList, updatePlan, addPlan, importPlanDetails, exportPlanDetails } from '@/api/plan'
  ...
  methods: {
      ...
      getPlanDetail(id).then(response => {
        ...
        if (attachment) {
          this.fileList = attachment.split(',').map(item => {
            ...
            return { name: originName, url: '', response: { originName, filename } };
          })
        } else {
          this.fileList = []
        }
    ...
    submitForm(type) {
      this.$refs.form.validate(valid => {
        ...
        updatePlan({ ...this.form, createTime, details, status }).then(() => {
